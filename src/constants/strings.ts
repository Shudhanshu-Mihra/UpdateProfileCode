export const STRINGS = {
  sign_in_up: {
    projectName: 'ReceiptHub',
    title: 'Receipt',
    hub: 'Hub',
    sub_title: `Manage Your Documents Fast and Easy`,
    sign_in: 'Sign in',
    sign_up: 'Sign up',
    email_input: 'Email',
    password_input: 'Password',
    full_name_input: 'Full Name',
    country_input: 'Country',
    show_password_icon: 'showPassword',
    forgot_password: 'Forgot Password?',
    continue_Capium: 'Continue with Capium',
    continue_Google: 'Continue with Google',
    without_acc: 'Don’t have an account?',
  },
};

export const EMPTY_DATA_STRINGS = {
  title: 'Uploaded receipts will be shown here...',
  firstSubtitle:
    'We have created everything for you, please upload your receipts to start.',
  secondSubtitle: 'You can do that by using Upload Receipt button below',
  buttonText: 'Upload Receipt',
};

export const EMPTY_DATA_STRINGS_MASTER = {
  categories: {
    title: 'Categories will be shown here...',
    firstSubtitle:
      'We have created everything for you, please add your categories to start.',
    secondSubtitle: 'You can do that by using Add Category button below',
    buttonText: 'Create Category',
  },
  suppliers: {
    title: 'Supplier accounts will be shown here...',
    titleSecond: 'Suppliers will be shown here...',
    firstSubtitle:
      'We have created everything for you, please add your supplier account to start.',
    secondSubtitle:
    'You can do that by using Create Supplier Account button below',
    thirdSubtitle:
      'We have created everything for you, please add your supplier to start.',
    fourthSubtitle:
      'You can do that by using Add Supplier button below',
    buttonText: 'Create Supplier Account',
  },
  customers: {
    title: 'Customer accounts will be shown here...',
    secondTitle: 'Customers will be shown here...',
    firstSubtitle:
      'We have created everything for you, please add your customers account to start.',
    secondSubtitle:
      'You can do that by using Create Customer button below',
      thirdSubtitle:
      'We have created everything for you, please add your customers to start.',
    fourthSubtitle:
      'You can do that by using Add Customer button below',
    buttonText: 'Create Customer Account',
  },
  type: {
    title: 'Payment types will be shown here...',
    firstSubtitle:
      'We have created everything for you, please add your payment type to start.',
    secondSubtitle: 'You can do that by using Create Payment Type button below',
    buttonText: 'Create Payment Type',
  },
  invites: {
    title: 'Invites will be shown here...',
    firstSubtitle: 'We have created everything for you',
    secondSubtitle: 'You can send invitations using the button below',
    buttonText: 'Create User',
  },
};

export const DATE_FORMATS = [
  {
    value: 'MMM-dd-yyyy',
    label: 'MMM-dd-yyyy',
  },
  {
    value: 'dd-MMM-yyyy',
    label: 'dd-MMM-yyyy',
  },
  {
    value: 'dd-MM-yy',
    label: 'dd-MM-yy',
  },
  {
    value: 'MM/dd/yyyy',
    label: 'MM/dd/yyyy',
  },
];

export const USER_ROLES = [
  {
    value: 'admin',
    label: 'Admin',
  },
  {
    value: 'user',
    label: 'Regular User',
  },
  {
    value: 'accountant',
    label: 'Accountant',
  },
  {
    value: 'owner',
    label: 'Company Owner',
  },
];

export const MAX_FILE_SIZE = 62914560; //13053968;
